...
window.onload=createCustomersContent();const buttons=document.querySelectorAll(".nav-button");const container=document.querySelector(".container");buttons.forEach(button=>{button.addEventListener("click",()=>{buttons.forEach(btn=>{btn.classList.remove("nav-button-active");});button.classList.add("nav-button-active");if(button.getAttribute("data-name")!=="customers"){container.innerHTML='<div class="oops-title">Oops, something went wrong</div>';}else{createCustomersContent()}});});function createCustomersContent(){renderCustomersContent();fetchDataFromFirebase().then(data=>{const searchInput=document.querySelector(".i-text");const tableBody=document.querySelector(".table-main");const itemsPerPage=8;displayTableData(data,currentPage,itemsPerPage);generatePaginationButtons(data,itemsPerPage);searchInput.addEventListener("input",event=>{const searchValue=event.target.value.toLowerCase();tableBody.innerHTML="";const filteredData=Object.values(data).filter(item=>{const customerName=item["Customer Name"].toLowerCase();return customerName.includes(searchValue);});currentPage=1;displayTableData(filteredData,currentPage,itemsPerPage);generatePaginationButtons(filteredData,itemsPerPage);});});}
function renderCustomersContent(){const container=document.querySelector(".container");const html=`
...
function displayTableData(data,currentPage,itemsPerPage){const startIndex=(currentPage-1)*itemsPerPage;const endIndex=startIndex+itemsPerPage;const dataArray=Object.values(data).slice(startIndex,endIndex);const tableBody=document.querySelector(".table-main");tableBody.innerHTML="";dataArray.forEach(item=>{const row=createTableRow(item);tableBody.appendChild(row);});updateShowingDataInfo(currentPage,itemsPerPage,Object.values(data).length);}
...
const nextButton=createPaginationButton("pagination-next","Next","chevron.svg");paginationList.appendChild(nextButton);prevButton.addEventListener("click",()=>{if(currentPage>1){currentPage--;displayTableData(data,currentPage,itemsPerPage);updatePaginationButtons();}});nextButton.addEventListener("click",()=>{if(currentPage<totalPages){currentPage++;displayTableData(data,currentPage,itemsPerPage);updatePaginationButtons();}});const pageButtons=Array.from(paginationList.querySelectorAll(".pagination-button"));pageButtons.forEach(button=>{button.addEventListener("click",()=>{const pageNumber=parseInt(button.textContent);if(pageNumber!==currentPage){currentPage=pageNumber;displayTableData(data,currentPage,itemsPerPage);updatePaginationButtons();}});});function updatePaginationButtons(){pageButtons.forEach(button=>{const pageNumber=parseInt(button.textContent);if(pageNumber===currentPage){button.classList.add("btn-pagination-active");}else{button.classList.remove("btn-pagination-active");}});}
updatePaginationButtons();}
function updateShowingDataInfo(currentPage,itemsPerPage,totalItems){const startIndex=(currentPage-1)*itemsPerPage;const endIndex=Math.min(startIndex+itemsPerPage,totalItems);const showingDataInfo=document.querySelector(".showing-data");showingDataInfo.textContent=`Showing data ${startIndex + 1} to ${endIndex} of ${totalItems} entries`;}
const openMobileMenuBtn=document.getElementById('open-mobile-menu');const closeMobileMenuBtn=document.getElementById('close-menu');openMobileMenuBtn.addEventListener('click',function(){openMobileMenu();});closeMobileMenuBtn.addEventListener('click',function(){closeMobileMenu();});function openMobileMenu(){const sidebar=document.querySelector('.sidebar');sidebar.classList.add('active');}
function closeMobileMenu(){const sidebar=document.querySelector('.sidebar');sidebar.classList.remove('active');}